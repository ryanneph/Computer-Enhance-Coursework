...
void print_f64_table(const char *name, f64 *table, int row_count) {
   u32 min_order = 0;
   int column_count = row_count-1;

   printf("%s : [%d][%d]float64 : .[\n", name, row_count, column_count);
   printf("    // NOTE(casey): This minimax coefficient table was donated by Demetri Spanos\n");
   for (int i = 0; i < row_count; i++) {
      if (i) printf("\n");
      printf("    .[");

      int order_zero_count = 0;
      for (int j = 0; j < column_count; j++) {
         f64 val = *(table + i*(column_count) + j);
         ...
         if (val == 0.0) ++order_zero_count;
      }
      printf("],");

      if (min_order == i && order_zero_count == column_count) ++min_order;
   }
   printf("\n];\n");
   printf("%s_MIN_ORDER: u64 : %d;\n", name, min_order);
   printf("%s_MAX_ORDER: u64 : %d;\n", name, row_count-1);
   printf("\n");
}
...
int main() {
   printf("// File output generated by %s\n\n", __FILE_NAME__);
